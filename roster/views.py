...
class SecuenciaRolViewSet(viewsets.ModelViewSet):
  ...
  queryset = (
      SecuenciaRol.objects.all().prefetch_related('detalles').order_by('-created_at')
  )
...
class TurnoDiaViewSet(viewsets.ModelViewSet):
  ...
  def create(self, request, *args, **kwargs):
    ...
    Soporta el cambio manual de turno al hacer clic en la gradilla del Roster.
    """
    ...
    # Si el código de turno está vacío, limpiar/eliminar el registro de turno para ese día
  ...
  @action(
      detail=False, methods=['post'], url_path='previsualizar-carga-masiva'
  )
  def previsualizar_carga_masiva(self, request):
    ...
    if not all(
        [tipo, referencia_id, secuencia_id, fecha_inicio_str, fecha_fin_str]
    ):
      return Response(
    ...
    if fecha_fin < fecha_inicio:
      return Response(
          {
              'error': (
                  'La fecha final debe ser posterior o igual a la fecha'
                  ' inicial.'
              )
          },
          status=status.HTTP_400_BAD_REQUEST,
    ...
    if tipo == 'operador':
      ...
      return Response(
          {
              'error': (
                  "Tipo de asignación inválido. Debe ser 'operador' o"
                  " 'cuadrilla'."
              )
          },
          status=status.HTTP_400_BAD_REQUEST,
  ...
  def carga_masiva(self, request):
    ...
    if not operadores:
      return Response(
          {
              'error': (
                  'No se encontraron colaboradores activos para aplicar la'
                  ' secuencia.'
              )
          },
          status=status.HTTP_400_BAD_REQUEST,
    ...
    try:
      ...
      return Response(
          {
              'error': (
                  'Error crítico durante la transacción atómica:'
                  f' {str(e)}'
              )
          },
    ...
    return Response(
        {
            'status': 'success',
            'mensaje': (
                'Carga masiva aplicada correctamente. Se generaron'
                f' {registros_creados} registros.'
            ),
